...
class RNNLM(NNBase):
    ...
    def _acc_grads(self, xs, ys):
        ...
        # Forward propagation
        for i in xrange(ns):
            hs[i] = sigmoid(dot(self.params.H, hs[i-1]) + self.sparams.L[xs[i]])
        ...
        for i in xrange(ns):
            score = ps[i].copy()
            score[ys[i]] -= 1
            self.grads.U += outer(score, hs[i])
            delta = dot(self.params.U.T, score) * hs[i] * (1-hs[i])
            for j in reversed(xrange(max(0, i-self.bptt), i+1)): 
                self.sgrads.L[xs[j]] = delta
                self.grads.H += outer(delta, hs[j-1])
                delta = dot(self.params.H.T, delta) * hs[j-1] * (1 - hs[j-1])
    ...
    def generate_sequence(self, init, end, maxlen=100):
        ...
        h = zeros(self.hdim)
        ns = 0
        while ys[-1] != end and ns < maxlen:
            h = sigmoid(dot(self.params.H, h) + self.sparams.L[ys[-1]])
            p = softmax(dot(self.params.U, h))
            ny = multinomial_sample(p)
            ys.append(ny)
            J -= log(p[ny])
            ns += 1
