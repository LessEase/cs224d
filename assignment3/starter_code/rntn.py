...
class RNTN:
    ...
    def initParams(self):
        ...
        # Gradients
        self.dV = np.zeros((self.wvecDim,2*self.wvecDim,2*self.wvecDim))
    ...
    def costAndGrad(self,mbdata,test=False): 
        ...
        return cost, [self.dL, self.dV+self.rho*self.V, self.dW+self.rho*self.W, self.db, self.dWs+self.rho*self.Ws, self.dbs]
    ...
    def forwardProp(self,node, correct=[], guess=[]):
        ...
        if node.isLeaf:
            ...
            costLeft, totalLeft = self.forwardProp(node.left)
            costRight, totalRight = self.forwardProp(node.right)
            hdown = np.hstack((node.left.hAct1, node.right.hAct1))
            hcur = np.dot(self.V, hdown).dot(hdown) + np.dot(self.W, hdown) + self.b
            hcur[hcur<0] = 0 
        ...
        node.probs = np.dot(self.Ws, node.hAct1) + self.bs
    ...
    def backProp(self,node,error=None):
        ...
        # Clear nodes
        node.fprop = False
        delta = np.zeros((self.numWords,))
        delta = node.probs.copy()
        delta[node.label] -= 1.0
        self.dbs += delta
        self.dWs += np.outer(delta, node.hAct1) 
        delta = np.dot(self.Ws.T, delta)
        ...
        delta_com = (node.hAct1>0) * delta
        #delta_com = delta

        hdown = np.hstack((node.left.hAct1, node.right.hAct1))
        self.dV += np.multiply(delta_com.reshape(self.wvecDim,1,1), np.outer(hdown, hdown).reshape(1, 2*self.wvecDim, 2*self.wvecDim))
        self.dW += np.outer(delta_com, hdown)
        self.db += delta_com
        delta_down = np.dot(self.W.T, delta_com)
        delta_down += np.multiply(self.V+self.V.transpose(0,2,1), delta_com.reshape(self.wvecDim,1,1)).sum(axis=0).dot(hdown)
        self.backProp(node.left, delta_down[:self.wvecDim])
        self.backProp(node.right, delta_down[self.wvecDim:])
