...
class RNN:
    ...
    def forwardProp(self,node,correct=[], guess=[]):
        ...
        if node.isLeaf:
            ...
            costLeft, totalLeft = self.forwardProp(node.left, correct, guess)
            costRight, totalRight = self.forwardProp(node.right, correct, guess)
            cost += costLeft
            cost += costRight
            total += totalLeft
            total += totalRight
            h = np.dot(self.W, np.hstack((node.left.hAct1, node.right.hAct1))) + self.b
            h[h<0] = 0
            node.hAct1 = h
        
        score = np.dot(self.Ws, node.hAct1) + self.bs
        
        score -= np.max(score)
        score = np.exp(score)
        
        node.probs = score/(np.sum(score))
        cost += -np.log(node.probs[node.label])
        correct.append(node.label)
        guess.append(np.argmax(node.probs, axis=0))
    ...
    def backProp(self,node,error=None):
        ...
        delta3 = np.copy(node.probs)
        delta3[node.label] -= 1.0  
       
        self.dbs += delta3
        self.dWs += np.outer(delta3, node.hAct1)
        
        deltas = np.dot(self.Ws.T, delta3)
        ...
        if node.isLeaf:
            ...
            self.db += deltas
            self.dW += np.outer(deltas, np.hstack((node.left.hAct1, node.right.hAct1)))
